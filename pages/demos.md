---
title: デモ
tags: [getting_started]
sidebar: home_sidebar
permalink: /demos/
summary: さきゅばすで録画されたコメント付き動画のデモ映像です。
---

# バージョン2.0β1

　2012年5月に公開されたバージョンでは、1からスクラッチで書き直し、ニワン語に対応しました。

## さきゅばす２でNicocococococo! の謎の技術を再現してみた

<video width="640" height="360" preload="metadata" controls="controls">
  <source type="video/mp4" src="/movies/sm17805115.mp4">
  <a href="/movies/sm17805115.mp4">/movies/sm17805115.mp4</a>
</video>

 Nicocococococo!([sm13485376](http://www.nicovideo.jp/watch/sm13485376))のニコニコテレビちゃんが可愛かったので再現してみました！
 ニワン語のプログラムを独自に解釈することで再現しています。簡単にいうと、まあエミュレータみたいな感じでしょうか。
 でもなんかアンテナズレてる。

 - 使用バージョン：2.0β1

## 【ニコニコ動画】【超会議】新さきゅばすでニワン語ゲームを再現してみた【比較】

<video width="640" height="360" preload="metadata" controls="controls">
  <source type="video/mp4" src="/movies/sm17638768.mp4">
  <a href="/movies/sm17638768.mp4">/movies/sm17638768.mp4</a>
</video>

[ニコニコ超会議](http://www.chokaigi.jp/)内で開催された[第二回ニコニコ学会β](http://niconicogakkai.jp/nng2/)で発表しました。なかなか楽しかったな。

> 超会議二日目、「ニコニコ学会β」の「コメントアート」セッションにて発表！
> [http://niconicogakkai.jp/nng2/](http://niconicogakkai.jp/nng2/)
> 
> ニコニコ動画コメント付動画作成ツール、「さきゅばす」の新バージョンのデモです。
> [http://saccubus.osdn.jp/](http://saccubus.osdn.jp/)
> ニワン語の独自インタプリタ「猫又」を搭載し、ニワン語／ニコスクリプトを使った動画の「コメント付き動画」も作成できるようになりました！
> 
> この新バージョンのさきゅばすを利用して、ProjectDIVA風ゲーム、Project Niva（sm14097905）を再現した結果をブラウザと比べました。静的に変換してるのでサクサク動きます。
>
> 開発者なども募集しています。言語処理系に興味のある方はぜひ。


## 【ニコニコ動画】【超会議】　新しいさきゅばすで、ニワン語のゲームのTAS作ってみた

<video width="640" height="360" preload="metadata" controls="controls">
  <source type="video/mp4" src="/movies/sm17633712.mp4">
  <a href="/movies/sm17633712.mp4">/movies/sm17633712.mp4</a>
</video>

　１フレーム送りで変換する「TASモード」を搭載し、スーパープレイ動画を作ることもできました。

> ニコニコ動画コメント付動画作成ツール、「さきゅばす」の新バージョンのデモです。
> [http://saccubus.osdn.jp/](http://saccubus.osdn.jp/)
> ニワン語の独自インタプリタ「猫又」を搭載し、ニワン語／ニコスクリプトを使った動画の「コメント付き動画」も作成できるようになりました！
>
> 今回は、この新さきゅばすを利用して、ProjectDIVA風ゲーム、Project Niva（sm14097905）
のHARDモードTAS(Tool Assisted Superplay)を作成してみました。
>
> TASさんのニコニコでの新たな戦いにご期待ください。
>
> ロケ地：スタバ（開発に使ってたら顔を覚えられた

# バージョン1

　2007年6月に公開された最初のバージョンのサンプルです。動画がカクカクしてしまっているのは動画アップロード時の問題で、実際に変換済みの動画ではなめらかに動作していました。ほんとだよ。

## 【ニコニコ動画】「さきゅばす」サンプル動画：きしめん

<video width="640" height="480" preload="metadata" controls="controls">
  <source type="video/mp4" src="/movies/sm515667.mp4">
  <a href="/movies/sm515667.mp4">/movies/sm515667.mp4</a>
</video>

> ニコニコ動画をコメント付きで保存できる自作ソフト「さきゅばす」で変換してみたきしめん動画のテストです。
>
> カオスバージョンはsm519949です。
> すいません、エンコードしたことが無いので画質は悪いです。。。もっとエンコード設定に凝れば、弾幕のところでもコマ落ちしないのに。

当時のニコニコ動画とクライアントPCのスペックでは、コメントがたくさんでる「弾幕」で処理落ちしてしまうのが常でした。さきゅばすの発想は、それを動画上にレンダリングしてしまうことで、この問題を解決しようというものでした。

…といいつつ、上でアップロードした動画はエンコードがへたくそだったので「弾幕」部分で結局コマ落ちしてしまっているんですが。ご愛嬌ってことで、ここは一つ。

## 【ニコニコ動画】「さきゅばす」で弾幕の制限をはずしてみた。

<video width="640" height="480" preload="metadata" controls="controls">
  <source type="video/mp4" src="/movies/sm519949.mp4">
  <a href="/movies/sm519949.mp4">/movies/sm519949.mp4</a>
</video>

　どれぐらいコメントを一画面内に描画できるかコントロールできるので、当時のニコニコ動画の最大の名物「弾幕」の制限を突破して無茶な数のコメントを描いたりして遊ぶことができました。

> 自作ソフト「さきゅばす」で変換したきしめん動画です。
> 
>通常１動画に付きコメント数が250件なのを1000件に無理やり増やして、一画面に表示可能なコメント数を通常の30件から512件に増やしました。
> 弾幕が恐ろしい事に・・・。SMILEVIDEOのデフォルトのエンコードなので、すさまじくコマ落ちしてますが、ちゃんとやればコマ落ちしません。
